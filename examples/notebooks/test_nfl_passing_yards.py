...
def test_history_excludes_same_week_and_unfinished_games():
    ...
    for week, day, value in [(1,1,10),(2,8,20),(2,9,999),(3,15,30)]:
        rows.append(dict(season=2025, week=week, game_id=str(day), checkpoint='q1',
            actual_qb_id='QB', team='A', opponent_team='B', game_date=f'2025-09-{day:02}',
            ...
            remaining_yards=value, official_passing_yards=value,
...
def test_q1_entry_blocks_halftime():
    rows=pd.DataFrame([dict(game_id='g',actual_qb_name='QB',checkpoint=h,ticker='t',
        side='yes',model_probability=p,entry_price=.2,fee=.02,edge=p-.22,
        settlement_value=1,reason='eligible') for h,p in [('q1',.5),('halftime',.8)]])
    ...
    combined=report[(report.checkpoint=='combined') & (report.allowance==.05)].iloc[0]
    assert abs(combined.profit-.73)<1e-8


def test_halftime_requires_matching_probability_confirmation():
    rows=pd.DataFrame([dict(game_id='g',actual_qb_name='QB',checkpoint=h,ticker='t',
        side='yes',model_probability=p,entry_price=price,fee=.02,edge=p-price-.02,
        settlement_value=0,reason='eligible') for h,p,price in [('q1',.5,.45),('halftime',.4,.2)]])
    assert select_strategy(rows)[1].empty
    rows.loc[1,'model_probability']=.6
    rows.loc[1,'edge']=.38
