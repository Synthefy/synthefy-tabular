...
from nfl_blog_inference import (modeling_rows, pearson, select_columns, IDENTITY_COLUMNS,
                                prediction_provenance, read_prediction_cache, write_prediction_cache)


def test_pairwise_missing_and_constant():
    assert pearson(np.array([1., 2., np.nan]), np.array([2., 4., 9.])) == 1.
    assert pearson(np.ones(3), np.arange(3.)) is None


def test_pruning_ties_and_constants():
    rows = pl.DataFrame({'remaining_passing_yards': [1., 2., 3., 4.],
                         'z': [1., 2., 3., 4.], 'a': [2., 4., 6., 8.],
                         'constant': [0., 0., 0., 0.]})
...
def test_context_start_and_eligibility():
    rows = pl.DataFrame({'season': [2017, 2018, 2025, 2025, 2026], 'week': [1]*5,
                         'game_id': ['a', 'b', 'c', 'd', 'e'], 'team': ['X']*5,
                         'live_decision_utc': [1, 2, 3, 4, 5],
                         'live_evaluation_eligible': [True, True, True, False, True],
                         'remaining_passing_yards': [1.]*5, 'official_passing_yards': [2.]*5})
    assert modeling_rows(rows)['game_id'].to_list() == ['b', 'c']



def test_resume_validates_inputs_and_output(tmp_path):
    query = pl.DataFrame({name: [1] for name in IDENTITY_COLUMNS}).with_columns(pl.lit(2.).alias('feature'))
    train = query
    columns = ['feature']
    provenance = prediction_provenance(train, query, columns, 'cpu')
    prediction = query.select(IDENTITY_COLUMNS).with_columns(
        pl.lit(1).alias('context_rows'), pl.lit('["feature"]').alias('selected_feature_columns_json'),
        *[pl.lit(1.).alias(name) for name in ('nori_mean', 'nori_p10', 'nori_median', 'nori_p90')],
        pl.lit([.1, .5, .9]).alias('nori_quantile_taus'),
        pl.lit([1., 2., 3.]).alias('nori_quantile_values'))
