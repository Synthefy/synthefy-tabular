...
def fixture():
    row = dict(
        ...
        live_anchor_utc="2025-09-01T00:30:00+00:00",
        live_decision_utc="2025-09-01T00:32:00+00:00",
        team="A",
        opponent_team="B",
        actual_qb_id="qb",
        actual_qb_name="Test QB",
        live_qb_passing_yards=50.0,
        official_passing_yards=250.0,
        nori_median=250.0,
        nori_quantile_taus=[0.1, 0.5, 0.9],
        nori_quantile_values=[150.0, 250.0, 350.0],
    )
    market = dict(
        ticker="TEST",
        title="Test QB records 200+ passing yards",
        floor_strike=199.5,
        expected_expiration_time="2025-09-01T04:00:00+00:00",
        created_time="2025-08-30T00:00:00+00:00",
        open_time="2025-08-30T00:00:00+00:00",
        close_time="2025-09-01T04:00:00+00:00",
        result="yes",
    )
    quote = dict(quote_ts=int(datetime(2025, 9, 1, 0, 32, tzinfo=timezone.utc).timestamp()), yes_bid=0.4, yes_ask=0.45)
    return row, market, quote


def replay(row, market, q, half=None, first_quote=None):
    return strategy.replay_blog_strategy(
        pl.DataFrame([row]),
        pl.DataFrame([half or row]),
        [market],
...
def test_q1_priority_and_fees():
    ...
    assert d["entry_price"][0] == 0.45 and d["fee"][0] == 0.02
    assert r["capital"] == pytest.approx(0.52)
    assert r["pnl"] == pytest.approx(0.48)
...
def test_lower_half_probability_rejected():
    row, m, q = fixture()
    half = {**row, "nori_quantile_values": [100.0, 200.0, 300.0]}
    q = {**q, "yes_bid": 0.2, "yes_ask": 0.25}
...
def test_q1_strike_selected_by_market_not_largest_edge():
    ...
    low_q = {**q, "yes_bid": 0.1, "yes_ask": 0.15}
    matched, _ = strategy.match_live_markets_to_predictions([m, low], pl.DataFrame([row]))
    from types import SimpleNamespace

    d, _, _ = strategy.run_live_primary_strategy(
        matched,
        pl.DataFrame([row]),
        {"TEST": q, "LOW": low_q},
        SimpleNamespace(contracts_per_qb_game=1, primary_min_net_edge=0.10, max_spread=0.10),
        SimpleNamespace(maximum_quote_age_minutes=5),
        0.07,
        line_policy="market_balanced",
    )
...
def test_ten_cent_decimal_spread_is_inclusive():
    assert strategy.bid_ask_spread(0.45, 0.55) == 0.10
    row, m, q = fixture()
    d, r = replay(row, m, {**q, "yes_bid": 0.45, "yes_ask": 0.55})
    assert r["bets"] == 1
    ...
    class Client:
        def get(self, path, **params):
            ...
            return {
                "candlesticks": [
                    {
                        "end_period_ts": q["quote_ts"],
                        "yes_bid": {"close_dollars": "0.12"},
                        "yes_ask": {"close_dollars": "0.21"},
                    },
                    {
                        "end_period_ts": q["quote_ts"] + 60,
                        "yes_bid": {"close_dollars": "0.90"},
                        "yes_ask": {"close_dollars": "0.91"},
                    },
                ]
            }

    result = strategy.quote_after_anchor(Client(), {**m, **row})
    assert result["yes_ask"] == 0.21 and result["yes_bid"] == 0.12


@pytest.mark.parametrize("raw", [{"close_dollars": "0.21"}, {"close": "0.21"}, {"close": 21}])
def test_quote_price_formats(raw):
    assert strategy._close_value(raw) == 0.21
